...
exports.setCustomUserClaimsFirebase = functions.https.onCall(async (data, context) => {
  ...
  let callerClaims = context.auth.token || {};
...
exports.createOrganizationAdmin = functions.https.onCall(async (data, context) => {
    ...
    console.log('[createOrganizationAdmin] App Check token verification status (context.app):', util.inspect(context.app, { depth: null }));
    ...
    if(context.auth) {
        ...
        if(context.auth.token && typeof context.auth.token === 'object') {
...
exports.removeAdminFromOrganizationFirebase = functions.https.onCall(async (data, context) => {
    ...
    if (callerClaims.role !== 'super_admin') {
        ...
        if (context.auth && context.auth.token) {
            console.log('[removeAdminFromOrganizationFirebase] Denied token claims (inspected):', util.inspect(context.auth.token, { depth: null }));
        }
    ...
    if (currentClaims.role === 'admin' && currentClaims.organizationId === organizationId) {
        const newClaims = { 
            role: 'collaborator', // Demote to collaborator in the same organization
            organizationId: organizationId 
        }; 
        ...
        try {
            const userDocRef = admin.firestore().collection('users').doc(userId);
            console.log(`[removeAdminFromOrganizationFirebase] Attempting to update Firestore document for UID ${userId} with role: ${newClaims.role}, organizationId: ${newClaims.organizationId}.`);
        ...
        return { success: true, message: `Administrador ${userId} teve seus privilégios removidos da organização ${organizationId} e foi rebaixado para colaborador.` };
