...
export default function EmployeeLayout({ children }: EmployeeLayoutProps) {
  ...
  React.useEffect(() => {
    const setupNotifications = async () => {
        if (isGuest) { // Skip setup if in guest mode
  ...
  const getCurrentTitle = () => {
    ...
    if (pathname === '/colaborador/dashboard' || pathname === '/colaborador') return 'Visão Geral'; // Handle root /colaborador
  ...
  return (
    <TooltipProvider>
      {/* Mobile-first layout: Header, Main Content, Bottom Nav */}
      <div className="flex flex-col min-h-screen w-full bg-gradient-to-b from-sky-50 via-white to-gray-100 dark:from-slate-900 dark:via-slate-950 dark:to-slate-800">
        ...
        <main className="flex-1 overflow-y-auto p-4 pb-20"> {/* Changed overflow-auto to overflow-y-auto */}
             {children}
        </main>
